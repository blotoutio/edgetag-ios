...
class StorageHandler {
    ...
    func saveStorageValues(storage: [AnyHashable: Any]) {
        UserDefaults.standard.set(storage, forKey: Constants.storageParameter)
    }

    func getStorageValues() -> [AnyHashable: Any] {
        return UserDefaults.standard.object(forKey: Constants.storageParameter) as? [AnyHashable: Any] ?? [:]
    }

    func saveConsentValues(consentValues: [String: Bool]) {
        UserDefaults.standard.set(consentValues, forKey: Constants.consent)
    }

    func getConsentValues() -> [String: Bool] {
        return UserDefaults.standard.object(forKey: Constants.consent) as? [String: Bool] ?? [:]
    }
