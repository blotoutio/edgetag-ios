...
public class NetworkManager {
    ...
    func sendAppInstallEvent() {
        if !StorageHandler.shared.getAppInstallEventSent() {
            ...
            self.addTag(isSystemEvent: true, withData: [:], eventName: Constants.appInstall, providers: [:]) { success, _ in
                if success {
    ...
    func sendInternalConsentForALL() {
        ...
        giveConsentForProviders(consent: consentValues) { success, _ in
            if success {
    ...
    public func giveConsentForProviders(consent: [String: Bool], completion: @escaping (_ success: Bool, _ error: Error?) -> Void) {
        ...
        router.request(.consent(consent: consent, storage: updatedStorageDict, userAgent: useragent, cookieStr: cookieHeader, pageURL: pageURL )) { _, response, error in
    ...
    func getStorageModelWithUserData() -> [AnyHashable: Any] {
    ...
    public func addTag(isSystemEvent: Bool? = false,
                       withData: [AnyHashable: Any], eventName: String, providers: [String: Bool], completion: @escaping (_ success: Bool, _ error: Error?) -> Void) {
        if !isSDKInitialized {
            let error = UserKeyError.sdkUninitialized
            completion(false, error)
            print("\(error.rawValue)")
        ...
        router.request(.tag(withData: withData, eventName: eventName, providers: providers, storage: updatedStorageDict, userAgent: useragent, cookieStr: cookieHeader, pageURL: pageURL )) { _, response, error in
    ...
    public func addUserIDGraph(userKey: String, userValue: String, completion: @escaping (_ success: Bool, _ error: Error?) -> Void) {
        ...
        router.request(.user(idGraphKey: userKey, idGraphValue: userValue, storage: updatedStorageDict, userAgent: useragent, cookieStr: cookieHeader, pageURL: pageURL)) { _, response, error in
    ...
    public func addDataIDGraph(idGraph: [String: String], completion: @escaping (_ success: Bool, _ error: Error?) -> Void) {
        ...
        router.request(.data(idGraph: idGraph, storage: updatedStorageDict, userAgent: useragent, cookieStr: cookieHeader, pageURL: pageURL)) { _, response, error in
    ...
    public func getDataForIDGraphKeys(idGraphKeys: [String], completion: @escaping (_ success: Bool, _ error: Error?, _ idGraph: [String: String]?) -> Void) {
        if !isSDKInitialized {
            let error = UserKeyError.sdkUninitialized
                    ...
                    if data != nil {
                        do {
                            ...
                            if let jsonResultObj = jsonDict?["result"] {
                                completion(true, nil, jsonResultObj as? [String: String])
                            } else {
                                completion(true, nil, [:])
                            }
                        } catch {
